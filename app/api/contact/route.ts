...
import { createInquiry, isDatabaseAvailable } from "@/lib/db";
import { cleanText, validEmail } from "@/lib/validation";
import { allowRequest, requestAddress } from "@/lib/rate-limit";

export async function POST(request: Request) {
  if (!isDatabaseAvailable()) return NextResponse.json({ error: "Persistent storage is temporarily unavailable." }, { status: 503 });
  ...
  try {
    ...
    return NextResponse.json({ error: "The request could not be stored. Please try again." }, { status: 503 });
  }
}
